...
{
	if (parser->filename)
		region_recycle(parser->rr_region, (void *)parser->filename,
...
<incl>.+ 		{ 	
	char *tmp;
	struct dname *origin = parser->origin;
	int error_occurred = parser->error_occurred;
	...
	if (include_stack_ptr >= MAXINCLUDES ) {
		...
		if (tmp) {
			...
			*tmp = '\0';
			...
			origin = dname_parse(parser->rr_region, tmp + 1);
			if (!origin) {
		...
		if (strlen(yytext) == 0) {
			...
			char *filename = region_strdup(parser->rr_region, yytext);
